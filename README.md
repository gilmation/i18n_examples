...
This is the route that most frameworks take. In general this is the way that I would always tend to do it because it fits best with the way that most ORMs (Object Relational Mapper) work. You simply define a one to many relationship from the main (entity) table to the table that represents each translated field, the objects that represent these entities would then contain, for each translated field, a structure of the form:

    { "es" => "Hola", "en" => "Hello", "eu" => "Kaixo", "fr" => "Bonjour" }
...
A single table which is used for all the translations for all the tables

<h2 id="single-desc">Description</h2>
In this design a single table is used to store all the translations available to the system, which would mean that the system could quite easily have some sort of centralised translation administration tool. As you will be able to see from the example schema and data, related translations are grouped by using a common value in the translation.field column. 
